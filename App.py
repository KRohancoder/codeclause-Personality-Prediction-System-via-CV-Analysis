...
connection = pymysql.connect(db='rohan', user='root', passwd='poonam23', host='localhost', port=3306)
...
st.set_page_config(
    ...
    page_icon='SRA_Logo.jpg',
)


def run():
    st.title("Personality Prediction System via CV Analysis")
    ...
    # Create the DB
    db_sql = """CREATE DATABASE IF NOT EXISTS rohan;"""
    cursor.execute(db_sql)
    connection.select_db("rohan")
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'Hitesh' and ad_password == 'Hitesh':
                st.success("Welcome Hitesh")
                # Display Data
